...
// Run reads raw 500-byte IO output frames from ch and writes them to rotating
// parquet files. It does not return until ch is closed.
func Run(ch <-chan [500]byte) {
	for {
		cfg := config.Get()
...
func writeFile(ch <-chan [500]byte, cfg config.Config) (done bool) {
...
func buildSchema(cfg config.Config) (*parquet.Schema, map[string]int, error) {
	group := parquet.Group{
...
func makeRow(data []byte, cfg config.Config, colMap map[string]int, numCols int) parquet.Row {
...
func encodeValue(data []byte, f config.FieldDef) parquet.Value {
	off := f.Offset
	switch f.Type {
	case "sint":
		if off < len(data) {
